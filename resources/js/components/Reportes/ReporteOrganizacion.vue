...
export default {
  ...
  methods: {
    exportPDF() {
      ...
      var columns = [
        { title: "Cédula Juridica", dataKey: "identificacion" },
        { title: "Nombre Organización", dataKey: "nombre" },
        { title: "Número Teléfono", dataKey: "telefono" },
        { title: "Correo Organización", dataKey: "correo" },
      ];
      ...
      doc.text(" Reporte de organizaciones", 50, 68);
    ...
    cargar10(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReporteOrganizacion/solo10")
    ...
    cargar25(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReporteOrganizacion/solo25")
          .then(({ data }) => (this.filtroArray = data.data));
          this.exportPDF();
      }
    },
    cargar50(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReporteOrganizacion/solo50")
          .then(({ data }) => (this.filtroArray = data.data));
           this.exportPDF();
      }
    },
    cargar75(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReporteOrganizacion/solo75")
          .then(({ data }) => (this.filtroArray = data.data));
           this.exportPDF();
      }
    },
    cargar100(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReporteOrganizacion/solo100")
          .then(({ data }) => (this.filtroArray = data.data));
           this.exportPDF();
      }
    }
