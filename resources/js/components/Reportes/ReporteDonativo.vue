...
export default {
  ...
  methods: {
    exportPDF() {
      ...
      var columns = [
        { title: "Persona Donante", dataKey: "identificacionPersona" },
        { title: "Organización Donante", dataKey: "identificacionOrganizacion" },
        { title: "Tipo Donativo", dataKey: "tipo" },
        { title: "Detalles", dataKey: "detalle" },
        { title: "Fecha Donativo", dataKey: "fecha" },
        { title: "Estado Donativo", dataKey: "estado" },
      ];
      ...
      doc.text(" Reporte de donativos", 50, 68);
      ...
      doc.save("Reporte donativos.pdf");
    },
    cargar() {
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReportesDonativos/todo")
    ...
    cargar10(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReportesDonativos/solo10")
    ...
    cargar25(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReportesDonativos/solo25")
          .then(({ data }) => (this.filtroArray = data.data));
          this.exportPDF();
      }
    },
    cargar50(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReportesDonativos/solo50")
          .then(({ data }) => (this.filtroArray = data.data));
           this.exportPDF();
      }
    },
    cargar75(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReportesDonativos/solo75")
          .then(({ data }) => (this.filtroArray = data.data));
           this.exportPDF();
      }
    },
    cargar100(){
      if (this.$gate.isAdmin() || this.$gate.isUser()) {
        axios
          .get("/api/ReportesDonativos/solo100")
          .then(({ data }) => (this.filtroArray = data.data));
           this.exportPDF();
      }
    }
