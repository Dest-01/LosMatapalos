    ...
    <div v-else class="container-all wow fadeInLeft" data-wow-duration="2s">
      <div v-for="evento in eventos.data" :key="evento.id" class="container">
        <img
          ...
          height="100%"
          alt=""
          @click="verImagen(evento)"
          data-bs-toggle="modal"
          data-bs-target="#exampleModal"
        />
        <div class="descripcion">
          <span class="text">{{ $t("Nombre") }}: {{ evento.nombre }}</span>
...
export default {
  ...
  methods: {
    ...
    getResults(page = 1) {
      this.$Progress.start();
      axios
        .get("/api/actividadesPasadasCliente?page=" + page)
        .then(({ data }) => (this.eventos = data.data));
      this.$Progress.finish();
    },
    cargarEvento() {
      axios
        .get("/api/actividadesPasadasCliente")
        .then(({ data }) => (this.eventos = data.data));
      axios
        .get("/api/actividadesPasadasCliente/List")
        .then(({ data }) => (this.eventosTodos = data.data));
    },
  },
  created() {
    this.$Progress.start();
